...
class NewAI
  ...
  def set_flags(target)
      PBDebug.log_ai("Checking flags...")
      if !flags_set?(target)
        PBDebug.log_ai("No flags found.")
        PBDebug.log_ai("Setting flags...")
        off_move = 4
        prio = 0
        for i in target.moves
          if i.function == "051"
            $spam_block_flags[:haze_flag].push(target)
            PBDebug.log_ai("#{target.name} has been assigned Haze flag")
          end
          if i.statusMove?
            off_move -= 1
          end
        ...
        PBDebug.log_ai("Offensive Move Count: #{off_move}")
        PBDebug.log_ai("Priority Move Count: #{prio}")
        if off_move == 0
          $spam_block_flags[:no_attacking_flag].push(target)
          PBDebug.log_ai("#{target.name} has been assigned No Attacking Flag")
        end
        if off_move < 2
          $learned_flags[:should_taunt].push(target)
          PBDebug.log_ai("#{target.name} has been assigned Should Taunt flag")
        end
        if prio == 0
          $spam_block_flags[:no_priority_flag].push(target)
          PBDebug.log_ai("#{target.name} has been assigned No Priority flag")
        end
        $spam_block_flags[:flags_set].push(target)
       PBDebug.log_ai("End flag assignment.")
     else
      PBDebug.log_ai("Flags found.\nEnd flag search")
     end
   end
  ...
  def set_up_move_check_target(target)
    ...
    if @target && @move.function_code == "0AE"
      if @target.battler.lastRegularMoveUsed &&
        GameData::Move.exists?(@target.battler.lastRegularMoveUsed) &&
         GameData::Move.get(@target.battler.lastRegularMoveUsed).flags[/e/]
        @battle.moldBreaker = @user.has_mold_breaker?
  ...
  def pbPredictMoveFailure
    ...
    return true if AIHandlers.move_will_fail?(@move.function_code, @move, @user, self, @battle)
    return false
  end
  ...
  def pbPredictMoveFailureAgainstTarget
    # Move effect-specific checks
    return true if AIHandlers.move_will_fail_against_target?(@move.function_code, @move, @user, @target, self, @battle)
    ...
    return true if @move.move.pbImmunityByAbility(@user.battler, @target.battler)
  ...
  def pbGetMoveScore(targets = nil)
    ...
    if @trainer.has_skill_flag?("ScoreMoves")
      ...
      old_score = score
      score = AIHandlers.apply_move_effect_score(@move.function_code,
      ...
      score = AIHandlers.apply_general_move_score_modifiers(
         score, @move, @user, self, @battle)
    end
    score = score.to_i
    score = 0 if score < 0
  ...
  def pbGetMoveScoreAgainstTarget
    ...
    if @trainer.has_skill_flag?("ScoreMoves")
      ...
      old_score = score
      score = AIHandlers.apply_move_effect_against_target_score(@move.function_code,
      ...
      score = AIHandlers.apply_general_move_against_target_score_modifiers(
