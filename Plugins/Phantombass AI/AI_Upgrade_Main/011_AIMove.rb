...
class NewAI::AIMove
  ...
  def set_up(move)
    ...
    @ai.battle.moldBreaker ||= ["163",
                                "164","501"].include?(function_code)
  end
  ...
  def function_code;                 return @move.function;                end
  ...
  def pbTarget(user)
    return @move.pbTarget((user.is_a?(NewAI::AIBattler)) ? user.battler : user)
  end
  ...
  def rough_priority(user)
    ...
    if user.ability_active?
      ret = BattleHandlers.triggerPriorityChangeAbility(user.ability, user.battler, @move, ret)
  ...
  def base_power
    ...
    return AIHandlers.get_base_power(function_code,
  ...
  def rough_damage
    base_dmg = base_power
    return base_dmg if @move.is_a?(PokeBattle_FixedDamageMove)
    max_stage = PokeBattle_Battler::STAT_STAGE_MAXIMUM
    stage_mul = PokeBattle_Battler::STAT_STAGE_MULTIPLIERS
    stage_div = PokeBattle_Battler::STAT_STAGE_DIVISORS
    ...
    is_critical = crit_stage >= PokeBattle_Move::CRITICAL_HIT_RATIOS.length ||
                  PokeBattle_Move::CRITICAL_HIT_RATIOS[crit_stage] <= 2
    ##### Calculate user's attack stat #####
    if ["187",
        "164"].include?(function_code)
    ...
    if user.ability_active?
      ...
      when :TINTEDLENS
        ...
        BattleHandlers.triggerDamageCalcUserAbility(
    ...
    if target.ability_active?
      BattleHandlers.triggerDamageCalcTargetAbilityNonIgnorable(
        target.ability, user_battler, target_battler, @move, multipliers, base_dmg, calc_type
      )
    end
    if !@ai.battle.moldBreaker
    ...
    if user.item_active?
      ...
      when :LIFEORB
        ...
        BattleHandlers.triggerDamageCalcUserItem(
    ...
    if target.item_active? && target.item && !target.item.is_berry?
      BattleHandlers.triggerDamageCalcTargetItem(
  ...
  def rough_accuracy
    ...
    if @move.function == "070"
      ...
      ret -= 10 if @move.id == :SHEERCOLD && !user.has_type?(:ICE)
      return [ret, 0].max
    end
    ...
    stageMul = PokeBattle_Battler::ACC_EVA_STAGE_MULTIPLIERS
    stageDiv = PokeBattle_Battler::ACC_EVA_STAGE_DIVISORS
  ...
  def apply_rough_accuracy_modifiers(user, target, calc_type, modifiers)
    ...
    if user.item_active?
      if user.item == :ZOOMLENS
        ...
        BattleHandlers.triggerAccuracyCalcUserItem(
          user.item, modifiers, user_battler, target_battler, @move, calc_type
        )
      end
    end
    if target.item_active?
      BattleHandlers.triggerAccuracyCalcTargetItem(
    ...
    modifiers[:evasion_stage] = 0 if function_code == "0A9"   # Chip Away
    if @ai.trainer.medium_skill?
  ...
  def get_score_change_for_additional_effect(user, target = nil)
    ...
    return 5 if @move.addlEffect < 100 &&
                (Settings::MECHANICS_GENERATION >= 6 || function_code != "0A4") &&
