...
class NewAI::AIBattler
  ...
  def can_become_trapped?
    ...
    if ability_active? && BattleHandlers.CertainSwitchingUserAbility(ability, battler, @ai.battle)
      return false
    end
    if item_active? && BattleHandlers.CertainSwitchingUserItem(item, battler, @ai.battle)
      return false
    end
    ...
    @ai.each_foe_battler(side) do |b, i|
      if b.ability_active? &&
         BattleHandlers.TrappingTargetAbility(b.ability, battler, b.battler, @ai.battle)
        return false
      end
      if b.item_active? &&
         BattleHandlers.TrappingTargetItem(b.item, battler, b.battler, @ai.battle)
        return false
      end
    end
    return true
  ...
  def wants_status_problem?(new_status)
    ...
    if has_move_with_function?("07E")
      return true if [:POISON, :BURN, :PARALYSIS, :FROZEN].include?(new_status)
    end
    return false
  end
  ...
  def wants_ability?(ability = :NONE)
    ...
    ret = 0
    NewAI::BASE_ABILITY_RATINGS.each_pair do |val, abilities|
    ...
    if @ai.trainer.medium_skill?
      ret = AIHandlers.modify_ability_ranking(ability, ret, self, @ai)
    end
    return ret
  end
  ...
  def wants_item?(item)
    ...
    ret = 0
    NewAI::BASE_ITEM_RATINGS.each_pair do |val, items|
  ...
  def effectiveness_of_type_against_single_battler_type(type, defend_type, user = nil)
    ...
    if Effectiveness.ineffective_type?(type, defend_type)
      ...
      if @ai.battle.pbWeather == :StrongWinds && defend_type == :FLYING
        ret = Effectiveness::NORMAL_EFFECTIVE_MULTIPLIER
      end
    end
