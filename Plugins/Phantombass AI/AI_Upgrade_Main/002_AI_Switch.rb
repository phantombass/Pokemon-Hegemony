...
class NewAI
  ...
  def pbChooseToSwitchOut(force_switch = false)
    ...
    if force_switch
      ...
      should_switch = AIHandlers.should_switch?(@user, reserves, self, @battle)
      if should_switch && @trainer.medium_skill?
        should_switch = false if AIHandlers.should_not_switch?(@user, reserves, self, @battle)
      end
...
#===============================================================================
AIHandlers::ShouldSwitch.add(:perish_song,
...
# remove by switching.
#===============================================================================
AIHandlers::ShouldSwitch.add(:significant_eor_damage,
  proc { |battler, reserves, ai, battle|
...
# handler.
#===============================================================================
AIHandlers::ShouldSwitch.add(:cure_status_problem_by_switching_out,
  proc { |battler, reserves, ai, battle|
...
#===============================================================================
AIHandlers::ShouldSwitch.add(:wish_healing,
  proc { |battler, reserves, ai, battle|
...
#===============================================================================
AIHandlers::ShouldSwitch.add(:yawning,
  proc { |battler, reserves, ai, battle|
    ...
    if ai.trainer.high_skill?
      ...
      trapping = false
      ai.each_foe_battler(battler.side) do |b, i|
        next if b.ability_active? && BattleHandlers.CertainSwitchingUserAbility(b.ability, b.battler, battle)
        next if b.item_active? && BattleHandlers.CertainSwitchingUserItem(b.item, b.battler, battle)
        ...
        if battler.ability_active?
          trapping = BattleHandlers.TrappingTargetAbility(battler.ability, b.battler, battler.battler, battle)
          break if trapping
        end
        if battler.item_active?
          trapping = BattleHandlers.TrappingTargetItem(battler.item, b.battler, battler.battler, battle)
          break if trapping
        end
      end
...
#===============================================================================
AIHandlers::ShouldSwitch.add(:asleep,
  proc { |battler, reserves, ai, battle|
...
# Substitute.
#===============================================================================
AIHandlers::ShouldSwitch.add(:battler_is_useless,
  proc { |battler, reserves, ai, battle|
...
#===============================================================================
AIHandlers::ShouldSwitch.add(:foe_absorbs_all_moves_with_its_ability,
  proc { |battler, reserves, ai, battle|
...
# enough.
#===============================================================================
AIHandlers::ShouldSwitch.add(:absorb_foe_move,
  proc { |battler, reserves, ai, battle|
    next false if !ai.trainer.medium_skill?
    ...
    ai.each_foe_battler(battler.side) do |b, i|
      b.moves.each do |move|
        next if move.statusMove?
        m_power = move.baseDamage
        m_power = 100 if move.function == "070"
...
#===============================================================================
AIHandlers::ShouldSwitch.add(:sudden_death,
  proc { |battler, reserves, ai, battle|
...
# and powerful.
#===============================================================================
AIHandlers::ShouldSwitch.add(:high_damage_from_foe,
  proc { |battler, reserves, ai, battle|
    ...
    ai.each_foe_battler(battler.side) do |b, i|
     # next if (b.level - battler.level).abs > 5
      ...
      next if move_data.category == 2
      eff = battler.effectiveness_of_type_against_battler(move_data.type, b)
      next if !Effectiveness.super_effective?(eff) || move_data.base_damage < 70
      switch_chance = (move_data.base_damage > 90) ? 50 : 25
      big_threat = (ai.pbAIRandom(100) < switch_chance)
      break if big_threat
    end
    if $game_switches[LvlCap::Expert]
      ai.each_foe_battler(battler.side) do |b, i|
        for move in b.battler.moves
          eff = battler.effectiveness_of_type_against_battler(move.type, b)
          if Effectiveness.super_effective?(eff)
            if !battler.faster_than?(b) && move.rough_damage > battler.totalhp/2
              big_threat = true
            elsif battler.faster_than(b)
              for m in battler.moves
                if m.rough_damage < b.hp || m.rough_damage < b.totalhp / 2
                  if move.rough_damage > battler.hp
                    big_threat = true
                  end
                end
              end
            ...
            if move.rough_damage > battler.hp || move.rough_damage > battler.totalhp / 2
              big_threat = true
            end
          end
        end
...
#===============================================================================
AIHandlers::ShouldNotSwitch.add(:lethal_entry_hazards,
  proc { |battler, reserves, ai, battle|
    ...
    reserves.each do |pkmn|
      pkmn.moves.each do |move|
        reserve_can_remove_hazards = (move.function == "110")
...
# move.
#===============================================================================
AIHandlers::ShouldNotSwitch.add(:battler_has_super_effective_move,
  proc { |battler, reserves, ai, battle|
...
#===============================================================================
AIHandlers::ShouldNotSwitch.add(:battler_has_very_raised_stats,
  proc { |battler, reserves, ai, battle|
    next false if battle.rules["suddendeath"]
    stat_raises = 0
