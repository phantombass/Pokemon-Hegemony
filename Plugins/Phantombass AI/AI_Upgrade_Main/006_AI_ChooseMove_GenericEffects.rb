...
class NewAI
  ...
  def get_score_for_target_stat_raise(score, target, stat_changes, whole_effect = true,
    ...
    stat_changes.each_with_index do |stat, idx|
      ...
      increment = [increment, PokeBattle_Battler::STAT_STAGE_MAXIMUM - target.stages[stat]].min   # The actual stages gained
  ...
  def stat_raise_worthwhile?(target, stat, fixed_change = false)
    ...
    return true if target.has_move_with_function?("0ED",
                                                  "08E")
    case stat
    when :ATTACK
  ...
  def get_target_stat_raise_score_one(score, target, stat, increment, desire_mult = 1)
    ...
    if [:ACCURACY, :EVASION].include?(stat)
      stage_mul = PokeBattle_Battler::ACC_EVA_STAGE_MULTIPLIERS
      stage_div = PokeBattle_Battler::ACC_EVA_STAGE_DIVISORS
    end
    ...
    when :SPEED
      ...
      each_foe_battler(target.side) do |b, i|
        next if !b.has_move_with_function?("099")
        score -= 5 * inc_mult
      end
    ...
    each_foe_battler(target.side) do |b, i|
      next if !b.has_move_with_function?("08E")
      score -= 5 * increment * desire_mult
    end
    return score
  end
  ...
  def get_score_for_target_stat_drop(score, target, stat_changes, whole_effect = true,
    ...
    stat_changes.each_with_index do |stat, idx|
      ...
      decrement = [decrement, PokeBattle_Battler::STAT_STAGE_MAXIMUM + target.stages[stat]].min   # The actual stages lost
  ...
  def stat_drop_worthwhile?(target, stat, fixed_change = false)
    ...
    when :ACCURACY
      meaningful = false
      target.battler.moves.each do |m|
        meaningful = true if m.accuracy > 0 && !m.function == "070"
        break if meaningful
      end
      return false if !meaningful
    when :EVASION
  ...
  def get_target_stat_drop_score_generic(score, target, stat_changes, desire_mult = 1)
    ...
    # (Competitive, Defiant)
    if target.opposes?(@user) && BattleHandlers::AbilityOnStatLoss[target.ability]
      score -= 10
    end
    return score
  end
  ...
  def get_score_for_weather(weather, move_user, starting = false)
    ...
    each_battler do |b, i|
      ...
      when :Sun
        ...
        if b.has_move_with_function?("00E", "00E") ||
           (b.has_move_with_function?("0B3") &&
      ...
      if @trainer.medium_skill? && !b.has_active_item?(:UTILITYUMBRELLA)
        beneficial_abilities = {
          ...
          :Sandstorm => [:SANDFORCE, :SANDRUSH, :SANDVEIL, :SCALER],
          :Hail      => [:FORECAST, :ICEBODY, :SLUSHRUSH, :SNOWCLOAK],
          :Starstorm => [:STARSPRINT],
          :AcidRain  => [:TOXICRUSH,:ACIDDRAIN],
          :Eclipse   => [:NOCTEMBOOST],
          :Windy     => [:BACKDRAFT]
        }[weather]
  ...
  def get_score_for_terrain(terrain, move_user, starting = false)
    ...
    each_battler do |b, i|
      ...
      when :Poison
        # Check for Poison moves
        if b.has_damaging_move_of_type?(:POISON)
          ret += (b.opposes?(move_user)) ? -10 : 10
        end
      end
    end
    ...
    if @trainer.medium_skill?
      ...
      each_battler do |b, i|
        ...
        # Moves
        if b.has_move_with_function?("0A4",
                                     "060",
                                     "18A",
                                     "0B3")
          ret += (b.opposes?(move_user)) ? -5 : 5
