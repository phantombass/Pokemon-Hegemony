...
module BattleHandlers
  ...
  def self.triggerOnOpposingStatGainItem(item, battler, battle, statUps, forced)
    return OnOpposingStatGainItem.trigger(item, battler, battle, statUps, forced)
  end
...
class PokeBattle_Battler
  ...
  def pbUseMove(choice,specialUsage=false)
    ...
    if @effects[PBEffects::GorillaTactics]  || @effects[PBEffects::ChoiceBand]
      ...
      loop do
        idx += 1
        break if @moves[idx].id == (@effects[PBEffects::GorillaTactics] || @effects[PBEffects::ChoiceBand])
      end
      if @moves[idx].pp == 0
        choice[1] = -1
        choice[2] = @battle.struggle
